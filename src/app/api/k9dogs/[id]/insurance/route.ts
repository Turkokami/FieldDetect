...
export async function GET(
  ...
  const records = await prisma.k9InsurancePayment.findMany({
    where: { dogId: id },
...
export async function POST(
  ...
  try {
    ...
    const record = await prisma.k9InsurancePayment.create({
      data: {
        dogId: id,
        provider: v.provider,
        policyNumber: v.policyNumber ?? null,
        amount: v.amount,
        coverageType: v.coverageType ?? null,
        dueDate: v.dueDate ? new Date(v.dueDate) : null,
        paidDate: v.paidDate ? new Date(v.paidDate) : null,
        notes: v.notes ?? null,
      },
    });
    ...
    console.error("[K9_INS_POST]", error);
