...
export async function GET(
  ...
  const records = await prisma.k9Certification.findMany({
    where: { dogId: id },
...
export async function POST(
  ...
  try {
    ...
    const record = await prisma.k9Certification.create({
      data: {
        dogId: id,
        name: v.name,
        certNumber: v.certNumber ?? null,
        issuedBy: v.issuedBy ?? null,
        issueDate: v.issueDate ? new Date(v.issueDate) : null,
    ...
    console.error("[K9_CERT_POST]", error);
