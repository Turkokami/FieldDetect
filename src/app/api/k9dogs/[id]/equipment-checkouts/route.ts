...
export async function GET(
  ...
  try {
    ...
    const checkouts = await prisma.equipmentCheckout.findMany({
      where: { dogId, user: { organizationId: user.organizationId } },
      include: {
    ...
    console.error("[DOG_CHECKOUT_GET]", error);
...
export async function POST(
  ...
  try {
    ...
    // Auto-include all active org equipment items if none specified
    ...
    console.error("[DOG_CHECKOUT_POST]", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
