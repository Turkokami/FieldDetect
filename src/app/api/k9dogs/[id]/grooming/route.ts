...
const createSchema = z.object({
  ...
  groomerName: z.string().optional().nullable(),
...
export async function GET(
  ...
  const records = await prisma.k9GroomingAppointment.findMany({
    where: { dogId: id },
    orderBy: { date: "desc" },
  });
...
export async function POST(
  ...
  try {
    ...
    const record = await prisma.k9GroomingAppointment.create({
      data: {
        dogId: id,
        date: new Date(v.date),
        groomerName: v.groomerName ?? null,
    ...
    console.error("[K9_GROOM_POST]", error);
