  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        ...
        <span>/</span>
        <Link href={`/vehicles/${vehicle.id}`} className="hover:text-foreground transition-colors">
          {vehicle.name}
        </Link>
        <span>/</span>
      ...
      <h1 className="text-2xl font-bold text-foreground mb-6">Edit Vehicle</h1>
      <VehicleForm initial={JSON.parse(JSON.stringify(vehicle))} />
    </div>
  );
}
